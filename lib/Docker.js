...
{
    var dockerProcess = Spawn('docker', ['inspect', 'ndt:' + name]);
...
{
    var dockerProcess = Spawn('docker', ['ps', '-l', '-q']);
    var data = '';
...
{
    var dockerProcess = Spawn('docker', ['rmi', '--force', 'ndt:' + name]);
...
{
    var args = ['run', '-i', 'ndt:' + name, '/bin/bash', '-c', cmd];
    var dockerProcess = Spawn('docker', args);
    var running = true;
...
{
    var args = ['run', '-i', '--rm', '-v', process.cwd() + ':/testing-src/:ro', 'ndt:' + name, '/bin/bash', '-c', cmd];
    var dockerProcess = Spawn('docker', args);
    var running = true;
...
Docker.commitContainer = function commitContainer(sha, name)
{
    ...
    var dockerProcess = Spawn('docker', ['commit', sha, 'ndt:' + name]);
...
Docker.makeNew = function makeNew(base, name, cmd, outputCallback)
{
    ...
    }).then(function (exists)
    {
        if (exists)
        {
            return Docker.removeContainer(name)
        }
    }).then(function ()
    {
