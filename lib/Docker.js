...
{
    var dockerProcess = Spawn('docker', ['inspect', name]);
...
{
    var dockerProcess = Spawn('docker', ['rmi', '--force', name]);
...
Docker.commitContainer = function commitContainer(sha, name)
{
    ...
    var dockerProcess = Spawn('docker', ['commit', sha, name]);
...
Docker.makeNew = function makeNew(name, cmd, outputCallback)
{
    ...
    {
        return exists ? name : Config['base-image'];
    }).then(function (base)
    {
