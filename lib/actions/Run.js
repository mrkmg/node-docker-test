...
{
    var ScreenObjects = {}, availableWidth, availableHeight, testHeight;

    availableHeight = screen.height;
    availableWidth = screen.width;

    // Todo: figure out how to work around this.
    testHeight = Math.floor(availableHeight / tests);

    if (testHeight < 4)
    {
        throw new Error('Screen not tall enough');
    }
    if (availableWidth < 40)
    {
        throw new Error('Screen not wide enough');
    }
    ...
    ScreenObjects.single = makeTestBox(screen, 0, availableHeight, availableWidth - 20);
    ...
    {
        ScreenObjects.tests.push(makeTestBox(screen, i * testHeight, testHeight, availableWidth - 20));
    }
...
function makeTestBox(holder, top, height, width)
{
    ...
    testBox.show = function () { testBox._holder.show() };
    testBox.hide = function () { testBox._holder.hide() };
