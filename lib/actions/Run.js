...
function Run(screen, render)
{
    ...
    }).then(function (versions)
    {
        ...
        square = Math.ceil(Math.sqrt(versions.length));
        width = Math.floor(100 / square);
        height = Math.floor(100 / (Math.ceil(versions.length / square)));
        ...
        for (var i = 0; i < Config.concurrency && i < versions.length; i++)
        {
            runNext();
        }
...
function setupScreen(screen, concurrentTests, totalTests)
{
    var ScreenObjects = {}, availableWidth, availableHeight, layout, availableColumns, availableRows;
...
function setupTest(screen, render, version, width, height, column, row)
{
    ...
    {
        return Docker.runContainerWithCopy('ndt:' + Config.name, Commands.test(version), function (data)
        {
