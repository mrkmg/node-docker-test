var util, Promise, EventEmitter, extend, pad;
...
RunCli.prototype.testFinished = function testFinished(test) {
    this.log(this._runnerId(test.runner), test.version, test.passed ? 'Passed' : 'Failed');
};
...
RunCli.prototype.showError = function showError(error) {
    console.error(error);
