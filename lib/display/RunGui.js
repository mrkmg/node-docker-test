...
RunGui.prototype.runnerStarted = function runnerStarted(runner) {
    ...
    self.testBoxes[runner].runnerStatus.setContent("Running");
};

RunGui.prototype.runnerFinished = function runnerFinished(runner) {
    var self = this;

    self.testBoxes[runner].runnerStatus.setContent("Stopped");
};
...
RunGui.prototype._addTestBox = function _addTestBox(top, height) {
    ...
    testBox.runnerStatus = Blessed.text({
        parent: testBox.title,
        width: 'shrink',
        right: 0,
        top: 0,
        inverse: true
